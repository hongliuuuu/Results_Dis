...
def mcode(ite):
    ...
    for ddd in range(4):
        if ddd ==0:
            X = IDHCodel
            Y = IDHCodelY
            testfile = open("NDIDHCodelwww4%f_%f.txt" % (R, ite), 'w')
        if ddd ==1:
            X = nonIDH
            Y = nonIDHY
            testfile = open("NDnonIDHwww4%f_%f.txt" % (R, ite), 'w')
        if ddd ==2:
            X = lowGrade
            Y = lowGradeY
            testfile = open("NDLGwww4%f_%f.txt" % (R, ite), 'w')
        if ddd ==3:
            X = Progression
            Y = ProgressionY
            testfile = open("NDprwww4%f_%f.txt" % (R, ite), 'w')
        Xnew1 = X[:, 0:1680]
        Xnew2 = X[:, 1680:3360]
        Xnew3 = X[:, 3360:5040]
        Xnew4 = X[:, 5040:6720]
